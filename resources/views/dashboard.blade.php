<x-app-layout>
    ...
    <div class="max-w-full mx-auto px-4 sm:px-6 lg:px-8 mt-8">
        ...
        @php
            ...
            $gridCols = $isAdmin ? 'md:grid-cols-1 lg:grid-cols-4' : 'md:grid-cols-2 lg:grid-cols-3';
        @endphp

        <div class="grid grid-cols-1 md: mx- {{ $gridCols }} gap-4 mb-6">
            <!-- Total Siswa -->
            <div class="bg-white rounded-xl p-4 border shadow-sm flex items-center w-full min-h-[80px]">
                ...
                <div class="ml-4 flex-1">
                    <h3 class="text-xs font-medium text-black-600">Total Siswa</h3>
                    <p class="text-xl font-bold text-gray-900">{{ $siswaCount ?? 0 }}</p>
                </div>
            </div>
            ...
            <div class="bg-white rounded-xl p-4 border shadow-sm flex items-center w-full min-h-[80px]">
                ...
                <div class="ml-4 flex-1">
                    <h3 class="text-xs font-medium text-black-600">Total Jurusan</h3>
                    <p class="text-xl font-bold text-gray-900">{{ $jurusanCount ?? 0 }}</p>
                </div>
            </div>

            <!-- Total Kelas -->
            <div class="bg-white rounded-xl p-4 border md: shadow-sm flex items-center w-full min-h-[80px]">
                <div class="p-3 rounded-full">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24"
                        stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <div class="ml-4 flex-1">
                    <h3 class="text-xs font-medium text-black-600">Total Kelas</h3>
                    <p class="text-xl font-bold text-black-900">{{ $kelasCount ?? 0 }}</p>
                </div>
            </div>
            ...
            @if(auth()->user()->role === 'admin')
                <div class="bg-white rounded-xl p-4 border shadow-sm flex items-center w-full min-h-[80px]">
                    ...
                    <div class="ml-4 flex-1">
                        <h3 class="text-xs font-medium text-gray-600">Total User</h3>
                        <p class="text-xl font-bold text-gray-900">{{ $userCount ?? 0 }}</p>
                    </div>
                </div>
            @endif
                    ...
                    <div class="flex flex-col sm:flex-row items-start sm:items-center p-3 rounded-lg hover:bg-gray-50 transition-colors duration-200 min-h-[70px]">
                        ...
                        <div class="ml-0 sm:ml-3 flex-1 min-w-0 w-full">
                            ...
                            @if(isset($act->subtitle))
                                <p class="text-xs text-gray-500 mt-1 break-words">{{ $act->subtitle }}</p>
                            @endif
                            <div class="flex flex-col sm:flex-row sm:items-center mt-2 gap-1 sm:gap-2 text-xs">
                                <span class="text-gray-400">{{ $act->created_at->format('d M Y H:i') }}</span>
                                <span class="hidden sm:block text-gray-300">•</span>
                                <span class="text-gray-400 sm:hidden">|</span>
                                <span class="text-gray-400">{{ $act->created_at->diffForHumans() }}</span>
                            </div>
                        </div>
                    </div>
                @empty
