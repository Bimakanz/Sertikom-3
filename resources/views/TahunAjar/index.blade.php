    ...
    <div class="px-4 py-6 sm:px-8">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6 gap-4">
            ...
            <div class="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
                <form method="GET" action="{{ route('tahunajar.index') }}" class="w-full sm:w-auto">
                    <div class="flex">
                        <input 
                            type="text" 
                            name="search" 
                    ...
                    @forelse ($data as $item)
                        <tr class="border-b hover:bg-gray-50">
                            ...
                            <td class="py-3 flex items-center gap-2">
                                ...
                                {{-- DELETE BUTTON --}}
                                <button
                                    onclick="showDeleteModal('{{ $item->id }}', '{{ $item->nama_tahun_ajar }}')"
                                    class="p-2 border rounded-lg hover:bg-gray-100 transition delete-btn"
                                    data-id="{{ $item->id }}"
                                    data-name="{{ $item->nama_tahun_ajar }}"
                                >
        ...
        <script>
            ...
            function showDeleteModal(id, name) {
                ...
                document.getElementById('deleteForm').action = "{{ route('tahunajar.destroy', ':id') }}".replace(':id', id);
